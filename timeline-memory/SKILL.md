...
```bash
python timeline-memory/scripts/timeline_cli.py <command> --store-root <path> [--input file.json] [--read-mode compat|strict]
```

只保留 4 个公开命令：

```bash
python timeline-memory/scripts/timeline_cli.py project-turn --store-root /path/to/store --input turn.json --read-mode compat
python timeline-memory/scripts/timeline_cli.py get-thread --store-root /path/to/store --thread-id thr_pay_bill --read-mode strict
python timeline-memory/scripts/timeline_cli.py list-threads --store-root /path/to/store --thread-kind task --status planned --read-mode strict
python timeline-memory/scripts/timeline_cli.py list-thread-history --store-root /path/to/store --thread-id thr_pay_bill --read-mode strict
```
