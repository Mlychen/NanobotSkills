...
- 项目使用时以 `SKILL.md` 为入口，不依赖先读 `README.md`
- 项目维护或首次接手时，建议先看 `README.md`
- 运行方式和测试入口看 `SKILL.md`
- 稳定公开 contract 以 `references/schema.md` 为准
- 设计过程和里程碑状态看 `references/design-milestones-status-and-acceptance.md`
- 风险排期和治理看 `references/risks/README.md`
...
稳定字段定义与边界以 `references/schema.md` 为准；本节只保留调用侧最常用摘要。
